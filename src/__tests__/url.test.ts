...
describe("validateServerUrl", () => {
  ...
  it("strips trailing path and slash", () => {
    expect(validateServerUrl("https://example.com/api/v1/")).toBe("https://example.com");
  });

  it("throws for ftp:// protocol", () => {
    expect(() => validateServerUrl("ftp://example.com")).toThrow("Unsupported protocol");
  });
...
describe("skillApiPath", () => {
  ...
  it("encodes special characters in slug", () => {
    expect(skillApiPath("my skill/test")).toBe("/api/skills/my%20skill%2Ftest");
  });
